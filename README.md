...
会期が未定なので、DAY 1／DAY 2 の相対表記で組んでいる。**各作品の実尺＋トーク30分**を積み上げて
時間が破綻しないことを確認したうえで、メインの Bill W. 2本を DAY 1 の午前・午後に置いた。

| | DAY 1（土） | DAY 2（日） |
|---|---|---|
| 11:00 | Bill W.（104分） | 一瞬の楽園（27分） |
| 13:30 | — | 嘘つきは〇○のはじまり（30分） |
| 14:30 | Bill W. Conscious Contact（58分） | — |
| 16:00 | — | アディクトを待ちながら（82分・クロージング） |
| 17:00 | 微熱（30分） | — |

programme.html のタイムテーブルと、各作品カードの時刻バッジは同じ内容を指している。
**開始時刻・上映順はいずれも構成案**で、確定情報ではない。